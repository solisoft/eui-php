...
EUI applications, written in PHP.
...
and a component model in which a view is an array and a handler changes
state. **No runtime dependencies** beyond `ext-openssl` — BLAKE3 is in here,
because an asset is named by the hash of its content and no PHP extension
ships one.

```php
<?php

require 'vendor/autoload.php';   // or src/autoload.php, without composer
...
final class Counter extends Component
{
    ...
    {
        return Dsl::column([
            Dsl::text((string) $this->count, ['size' => '4xl', 'weight' => 'bold']),
...
An array, and a pure function of the state. `EUI\Dsl` builds them, but there
is nothing behind the helpers: print one and you have the whole document.
...
- **`k`** one of the seventeen primitive kinds. Everything a person would
  ...
  client.
- **`s`** a flat style array in the spec's own vocabulary. An unknown key is
...
the same style array here. Also worth the visit:
...
in*. Sizes are scale indices, by index or by name: `'size' => 'lg'`,
`'gap' => 4`, `'radius' => 'md'`.
...
function that returns an array, so it ports by writing the same array.
...
```php
public function onPick(array $params): void
{
...
| | |
|---|---|
| `$app->run(port: 5097)` | plain `ws://` on loopback, for development |
| `$app->run(host: '0.0.0.0', port: 443, tls: ['cert' => …, 'key' => …])` | TLS 1.3, the protocol's floor |

One forked process per connection: a session is a loop that blocks on its own
socket, and PHP has no threads to give it. That shapes one thing worth
knowing — a component's `refresh()` is for a handler that changed something
the view has not been asked about yet, not a way for another process to push.
...
The publisher key is a PKCS#8 PEM, generated on first use and kept: a client
pins it against `app_id` on first run and refuses a different one later. It
belongs to the application, not to a deployment, and never in a repository.
...
96 of them, and the ones worth reading are `tests/proto_test.php` — the
spec's own §8 example, 150 bytes, byte for byte — `tests/apply_test.php`,
...
`tests/session_test.php`, which forks a real server on a real socket and
